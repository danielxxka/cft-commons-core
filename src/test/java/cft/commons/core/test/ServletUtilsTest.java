...
import cft.commons.core.util.ServletUtils;
...
public class ServletUtilsTest {
	...
	public void encodeParameterStringWithPrefix() {
		...
		String queryString = ServletUtils.encodeParameterStringWithPrefix(params, "search_");
		...
		params.put("name", "foo");
		params.put("age", 1);
		queryString = ServletUtils.encodeParameterStringWithPrefix(params, "search_");
		assertEquals("search_name=foo&search_age=1", queryString);

		// prefix is blank or null
		queryString = ServletUtils.encodeParameterStringWithPrefix(params, null);
		assertEquals("name=foo&age=1", queryString);

		queryString = ServletUtils.encodeParameterStringWithPrefix(params, "");
		assertEquals("name=foo&age=1", queryString);

		// map is empty or null
		queryString = ServletUtils.encodeParameterStringWithPrefix(null, "search_");
